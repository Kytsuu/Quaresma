...
import { ArrowRight, Sparkles, Cross, Loader2, Quote, ChevronRight, User as UserIcon } from 'lucide-react';
...
export const QuizFunnel: React.FC<QuizFunnelProps> = ({ onComplete }) => {
  ...
  const handleAnswer = async (profile: VirtueProfile) => {
    ...
    if (newAnswers.length === 2 || newAnswers.length === 4) {
      ...
      const feedbackRes = await generateQuizEncouragement(newAnswers.map(a => a as string));
  ...
  const generateFinalResult = async (answers: VirtueProfile[]) => {
    ...
    const diagnosticRes = await generateQuizDiagnostic(dominantProfile, answers.map(a => a as string));
  ...
  if (step === 'intro') {
    return (
      <div className="min-h-screen flex items-center justify-center p-4 bg-stone-50 overflow-hidden relative">
        <div className="max-w-2xl w-full bg-white rounded-[4rem] p-10 md:p-20 shadow-2xl border border-amber-100 relative z-10 text-center animate-in fade-in slide-in-from-bottom-10 duration-1000">
          <div className="bg-amber-50 w-24 h-24 rounded-[2.5rem] flex items-center justify-center mx-auto mb-10 border border-amber-100 shadow-inner">
            <Sparkles className="w-12 h-12 text-amber-600 animate-pulse" />
          </div>
          <h2 className="text-4xl md:text-6xl font-black font-serif italic text-stone-900 mb-8 leading-tight">
            Descubra sua Virtude Predominante
          </h2>
          <p className="text-stone-500 text-lg md:text-2xl font-light mb-12 max-w-lg mx-auto">
            Antes de entrar no deserto, entenda qual <span className="text-amber-700 font-bold">arma espiritual</span> Deus te deu para esta batalha.
          </p>
          <button
            onClick={() => setStep('questions')}
            className="group w-full bg-stone-950 hover:bg-black text-white py-7 rounded-[2rem] font-black uppercase tracking-[0.4em] text-[14px] shadow-2xl transition-all hover:-translate-y-1 flex items-center justify-center gap-4"
          >
        ...
        <div className="max-w-2xl w-full">
          <div className="mb-8 flex items-center justify-between px-6">
            <div className="flex gap-3">
              {questions.map((_, i) => (
                <div key={i} className={`h-2 w-16 rounded-full transition-all duration-700 ${i <= currentQuestionIndex ? 'bg-amber-500 shadow-lg' : 'bg-stone-200'}`} />
              ))}
            </div>
            ...
            {aiFeedback ? (
              <div className="flex flex-col h-full items-center justify-center text-center p-10 md:p-20 bg-stone-900 text-white relative">
                ...
                <div className="relative z-10 space-y-10">
                  ...
                  <p className="text-3xl md:text-5xl font-serif italic leading-relaxed font-bold px-4 max-w-xl mx-auto italic">
                    "{aiFeedback}"
                  </p>
              ...
              <>
                <div className="h-64 md:h-80 relative overflow-hidden shrink-0">
                  <img src={currentQuestion.imageUrl} className="w-full h-full object-cover transform hover:scale-110 transition-transform duration-[10s]" alt="Cenário" />
                  ...
                  <div className="grid gap-5">
                    {currentQuestion.options.map((option, i) => (
                      <button
                        ...
                        className="group flex items-center justify-between p-8 rounded-[2.5rem] border-2 border-stone-100 hover:border-amber-500 hover:bg-amber-50 transition-all text-left active:scale-[0.98]"
                      >
                        <span className="text-xl text-stone-700 font-medium">{option.text}</span>
  ...
  if (step === 'lead') {
    return (
      <div className="min-h-screen flex items-center justify-center p-4 bg-stone-50 text-center">
        <div className="max-w-xl w-full bg-white rounded-[4rem] p-10 md:p-16 shadow-2xl border border-emerald-100 animate-in fade-in slide-in-from-bottom-10 duration-1000">
          ...
          <p className="text-stone-500 mb-10 max-w-sm mx-auto">
            Como devemos te chamar no seu relatório profético?
          </p>
          <form onSubmit={handleLeadSubmit} className="space-y-6">
            <input
              ...
              className="w-full px-8 py-5 rounded-3xl border-2 border-stone-100 focus:border-emerald-500 outline-none text-lg"
            />
            <button
              type="submit"
              className="w-full bg-emerald-600 hover:bg-emerald-700 text-white py-6 rounded-3xl font-black uppercase tracking-[0.4em] text-[13px] shadow-2xl flex items-center justify-center gap-4"
            >
      ...
      <div className="min-h-screen flex flex-col items-center justify-center p-4 bg-stone-950 text-white">
        <div className="max-w-md w-full text-center space-y-16">
          ...
          <div className="space-y-6">
            <h4 className="text-3xl font-serif italic font-bold">Gerando Relatório de {leadName}...</h4>
            <div className="w-full h-1.5 bg-white/5 rounded-full overflow-hidden">
              <div className="h-full bg-amber-500 animate-progress"></div>
            </div>
          </div>
          <p className="text-stone-300 text-lg italic animate-pulse">
